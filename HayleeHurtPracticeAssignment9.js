...
const err = new notStringError(`This is not a string!`);
...
function joinAlpha(array){
  ...
  for (let i = 0; i < array.length; i++) {
    try{
      if (typeof array[i] !== 'string') throw new notStringError('Not a string!');
    } catch (err){
      console.ewrror(err.stack);
    }finally {
      if (typeof array[i] === 'string') newString += array[i];
    }
    
    }
  return newString;
  }
console.log(joinAlpha(eightArray));